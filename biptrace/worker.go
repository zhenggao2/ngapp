	...
	if p.pattern == ".pcap"  {
		...
		for _, file := range fileInfo {
			if !file.IsDir() && path.Ext(file.Name()) == ".pcap" {
				...
				cmd = exec.Command(path.Join(p.wsharkPath, BIN_TSHARK), "-r", path.Join(p.bipTracePath, file.Name()), "-X", fmt.Sprintf("lua_script:%s", path.Join(p.luasharkPath, LUA_LUASHARK)), "-P", "-V")
				cmd.Stdout = &stdOut
				cmd.Stderr = &stdErr
